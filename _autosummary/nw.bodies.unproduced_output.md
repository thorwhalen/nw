...
[`nw.graph.ProjectGraph.add_unproduced_output()`](nw.graph.md#nw.graph.ProjectGraph.add_unproduced_output) at the same choke point
...
output kind’s URI, exactly like [`nw.bodies.verifying_trace`](nw.bodies.verifying_trace.md#module-nw.bodies.verifying_trace) never
...
[`add_unproduced_output()`](nw.graph.md#nw.graph.ProjectGraph.add_unproduced_output) **dedupes on this key**:
...
the record is stale but still there). [`add_annotation()`](nw.graph.md#nw.graph.ProjectGraph.add_annotation)
...
write through [`add_annotation()`](nw.graph.md#nw.graph.ProjectGraph.add_annotation), as the module
docstring on [`nw.graph`](nw.graph.md#module-nw.graph) already requires for verifying traces.
...
1. **Parentless.** `was_derived_from` is empty; the link to what it
   ...
   `descendants_of` set, and — worse — [`nw.freshness`](nw.freshness.md#module-nw.freshness) would read
...
2. **Excluded from freshness by construction.** Because it is parentless it
   ...
   also excluded from [`nw.Project()`](nw.md#nw.Project)’s resumption-brief “last authored
...
`error`, [`add_unproduced_output()`](nw.graph.md#nw.graph.ProjectGraph.add_unproduced_output) stores a
