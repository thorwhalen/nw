...
| [`ResumptionBrief`](#nw.schema.ResumptionBrief)(\*\*data) | A "where we left off" snapshot, returned by [`nw.Project.resumption_brief()`](nw.md#nw.Project.resumption_brief). |
...
load-bearing rather than cosmetic: [`nw.Project.read_spec()`](nw.md#nw.Project.read_spec) builds
a `CharacterRef` from the graph body and
[`nw.Project.write_spec()`](nw.md#nw.Project.write_spec) writes the body back from the
...
A “where we left off” snapshot, returned by [`nw.Project.resumption_brief()`](nw.md#nw.Project.resumption_brief).
