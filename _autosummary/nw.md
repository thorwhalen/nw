...
informally called an “app” (see [`nw.genres`](nw.genres.md#nw.genres) and issue #10).
...
[`nw.bodies.CharacterRefBodyV1`](nw.bodies.md#nw.bodies.CharacterRefBodyV1) field-for-field, and that is
...
Mirrors [`nw.bodies.EnvironmentRefBodyV1`](nw.bodies.md#nw.bodies.EnvironmentRefBodyV1) field-for-field, for the
...
* **Return type:**
  [`ProjectSpec`](nw.schema.md#nw.schema.ProjectSpec)
...
Read [`ResumptionBrief`](nw.schema.md#nw.schema.ResumptionBrief) before trusting the numbers —
...
* **Return type:**
  [`SongInfo`](nw.schema.md#nw.schema.SongInfo)
...
[`nw.pricing.current_quote()`](nw.pricing.md#nw.pricing.current_quote) would rewrite history at today’s
...
[`nw.pricing.quote_render_decision()`](nw.pricing.md#nw.pricing.quote_render_decision) is what answers that (nw#74).
...
([`nw.bodies.verifying_trace`](nw.bodies.verifying_trace.md#module-nw.bodies.verifying_trace)) is recorded here rather than in
...
[`nw.bodies.unproduced_output`](nw.bodies.unproduced_output.md#module-nw.bodies.unproduced_output)’s module docstring for the key);
...
[`nw.bodies.unproduced_output`](nw.bodies.unproduced_output.md#module-nw.bodies.unproduced_output)’s module docstring for the full
...
* **Return type:**
  [`Secrets`](nw.secrets.md#nw.secrets.Secrets)
...
[`nw.graph.ProjectGraph.add_unproduced_output()`](nw.graph.md#nw.graph.ProjectGraph.add_unproduced_output) /
[`add_annotation()`](nw.graph.md#nw.graph.ProjectGraph.add_annotation) — [`BaseTransform`](#nw.BaseTransform)
already does.
...
`fan_out_execute()` and
[`nw.jobs.enqueue()`](nw.jobs.md#nw.jobs.enqueue) pass it accepts-it-or-not, so an override
that has no key to spend never sees it; [`BaseTransform`](#nw.BaseTransform) binds
a [`FAL_SECRET`](nw.secrets.md#nw.secrets.FAL_SECRET) as the fal credential for the
...
caller passed — run it through [`nw.secrets.as_secrets()`](nw.secrets.md#nw.secrets.as_secrets) before
logging or formatting it. See [`nw.secrets`](nw.secrets.md#module-nw.secrets).
...
* **Return type:**
  [`ValidationReport`](nw.validation.md#nw.validation.ValidationReport)
...
[`nw.fan_out_execute()`](#nw.fan_out_execute), [`nw.jobs.enqueue()`](nw.jobs.md#nw.jobs.enqueue) — run every incoming
...
[`nw.freshness`](nw.freshness.md#module-nw.freshness) documents as the safe one.)
...
[`nw.freshness`](nw.freshness.md#module-nw.freshness), so removing it changes no freshness answer.
...
* **Return type:**
  [`ComposeReport`](nw.inspect.md#nw.inspect.ComposeReport)
...
* **Return type:**
  [`PlanQuote`](nw.pricing.md#nw.pricing.PlanQuote)
...
contract as [`nw.freshness.stale_verdicts()`](nw.freshness.md#nw.freshness.stale_verdicts). The closure used to be
...
credential for the call ([`nw.secrets.using_secrets()`](nw.secrets.md#nw.secrets.using_secrets)) even for an
...
* **Return type:**
  [`Strategy`](nw.renderers.md#nw.renderers.Strategy)
...
Dispatches on `prep.shot.render_strategy` via [`nw.renderers`](nw.renderers.md#module-nw.renderers).
...
* **Parameters:**
  ...
  * **upload** ([`bool`](https://docs.python.org/3/builtins/functions.html#bool)) – When True (default), upload local files to fal-storage and
    ...
    are populated.
* **Return type:**
  [`ShotPreparation`](nw.workflow.md#nw.workflow.ShotPreparation)
* **Returns:**
...
| [`freshness`](nw.freshness.md#module-nw.freshness)                     | Freshness with **early cutoff** — what is *actually* out of date.                                                               |
|----------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------|
| [`inspect`](nw.inspect.md#module-nw.inspect)                         | QA helpers — typed reports about rendered shots.                                                                                |
| [`checks`](nw.checks.md#module-nw.checks)                           | The checks nw itself ships — a small, honest default menu.                                                                      |
| [`bodies`](nw.bodies.md#module-nw.bodies)                           | Body schemas for nw's project-graph annotations.                                                                                |
| [`delivery`](nw.delivery.md#module-nw.delivery)                       | What a genre hands back when a human wants to *hold* what it made.                                                              |
| [`experiment`](nw.experiment.md#module-nw.experiment)                   | Experiment helpers — clone projects, apply operations across siblings.                                                          |
| [`genres`](nw.genres.md#nw.genres)                                  | A typed dict-backed plugin registry.                                                                                            |
| [`graph`](nw.graph.md#module-nw.graph)                             | The project annotation graph — read/write helpers + reelee-style traversals.                                                    |
| [`graph_backend`](nw.graph_backend.md#module-nw.graph_backend)             | Config-driven backend selection for nw's annotation graph stores.                                                               |
| [`jobs`](nw.jobs.md#module-nw.jobs)                               | nw.jobs — a project-scoped async **job** facade over `au`.                                                                      |
| [`migrate`](nw.migrate.md#module-nw.migrate)                         | Idempotent migration: project.json (sections/shots/refs) → lacing graph.                                                        |
| [`pricing`](nw.pricing.md#module-nw.pricing)                         | Re-quoting a persisted plan at today's rates (nw#74).                                                                           |
| [`project`](nw.project.md#module-nw.project)                         | Project facade: a folder on disk → typed reads, typed writes, typed summary.                                                    |
| [`renderers`](nw.renderers.md#module-nw.renderers)                     | Render strategies — pluggable, plan-producing, **shot-typed**.                                                                  |
...
| [`storyboard`](nw.storyboard.md#module-nw.storyboard)                   | Storyboard ↔ Project bridge.                                                                                                    |
