...
- **It does not touch cache identity.** `cost_basis` is descriptive: falaw
  ...
  on the same digest, and [`nw.jobs.enqueue()`](nw.jobs.md#nw.jobs.enqueue)’s idempotency key is unmoved
  by anything here — `tests/test_pricing.py` pins that.
- **It does not re-quote money already spent.**
  [`nw.Project.total_spend_usd()`](nw.md#nw.Project.total_spend_usd) sums what was *billed*, and a receipt is
