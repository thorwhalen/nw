...
([`nw.Transform.execute()`](nw.md#nw.Transform.execute), or the render callable behind a
[`nw.jobs.enqueue()`](nw.jobs.md#nw.jobs.enqueue)) **without** going through the graph: a key must
...
passed accepts-it-or-not by [`nw.fan_out_execute()`](nw.md#nw.fan_out_execute) and by
[`nw.jobs.enqueue()`](nw.jobs.md#nw.jobs.enqueue)’s dispatch — the same seam `on_failure` (nw#25) and
...
| [`FAL_SECRET`](#nw.secrets.FAL_SECRET)   | `nw.BaseTransform.execute()` and the [`nw.jobs`](nw.jobs.md#module-nw.jobs) worker bind it as the fal credential (`falaw.using_fal_credentials()`) for the duration of the call.   |
|---------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
[`nw.jobs`](nw.jobs.md#module-nw.jobs) worker bind it as the fal credential
...
[`nw.fan_out_execute()`](nw.md#nw.fan_out_execute), [`nw.jobs.enqueue()`](nw.jobs.md#nw.jobs.enqueue) — run every incoming
